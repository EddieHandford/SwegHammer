...
3. **Band program on the squad frame (gated 7.05) — full plan in `docs/BAND_PROGRAM_PLAN.md`.** The residual is one systemic bias: the sim rewards killing / durability / objective-control-by-count and under-rewards soft win conditions. Cross-faction key: the matrix is closed, so raising the under-pole automatically LOWERS the over-pole — drive from the under-pole, never a direct elite nerf. Ranked: **P1 battle-shock** (Objective-Control→0 + action-ineligible + Shadow-in-the-Warp mortals; systemic, appears missing, lifts Tyranids/under-pole), **P2 reserves / deep-strike fidelity** (Daemons −21 + Genestealer Cults −20.5, the two biggest unders — 6" window, arrive-onto-objectives, Cult Ambush resurrection), **P3 resource economies** (Sororitas Miracle Dice, finish Chaos Space Marines Dark Pacts, Daemons Corrupt Realspace), **P4 action-economy calibration** (the model is ALREADY faithful — verify weighting + add Battle-shocked/Advanced eligibility; **NO Vehicle/Monster/Titanic action ban — that is a FABRICATION, only AIRCRAFT/OC-0/engaged are barred per Chapter Approved 2025-26**), **P5 positional over-pole** (the known OC-reach representation gap — re-measure on the squad frame, revisit last). One change per wave, re-read the full matrix + matchup variance, viable-options test. See [[project-faction-residual-rootcause]] and [[project-oc-contest-faithful]].
