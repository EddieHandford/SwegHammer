...
3. **Band program on the squad frame (gated 7.05) — full plan in `docs/BAND_PROGRAM_PLAN.md`.** The residual is one systemic bias: the sim rewards killing / durability / objective-control-by-count and under-rewards soft win conditions (action-secondary scoring, board control, reserves, resource economies). Ranked priorities: **P1 audit + complete the action economy** (the master lever — `_unit_can_perform_action` is partially built; verify the Vehicle/Monster/Titanic restriction + action-secondary weighting; moves BOTH poles), **P2 battle-shock → Objective Control 0** (systemic, Tyranids-skewed, appears missing), **P3 reserves / deep-strike fidelity** (Daemons −21 + Genestealer Cults −20.5, the two biggest unders), **P4 resource economies** (Sororitas Miracle Dice, finish Chaos Space Marines Dark Pacts, Daemons Corrupt Realspace), **P5 distance-gated auras** (trim Death Guard over-pole). One change per wave, re-read the full matrix + matchup variance, viable-options test. See [[project-faction-residual-rootcause]].
