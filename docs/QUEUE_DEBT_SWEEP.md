...
| 3 | **Fight-alternation re-test on the current frame** (`SWEG_FIGHTALT=1`) | wave 231 fresh-eyes T3.3 | NEVER-EXECUTED — rejected waves 166/168 on a ~60-wave-stale frame predating collision / reserves cap / deck re-align / per-model / Chaos Space Marines work | A/B only (code built) | Converges with structural review surface 4 item 1. Paired eval after the wave-234 anchor lands. |
