...
\begin{tikzpicture}[
    ...
    goalbox/.style={
      ...
      fill=greentick!15, text width=110mm, align=center,
      minimum height=24mm, inner sep=5pt
    },
  ...
  \node[oneliner, below=3mm of heading] (oneliner) {%
    ...
    the game the way real tournaments do. Step two: use it to fit
    one points equation that prices every unit fairly from its stats.%
  };
  ...
  \node[stageheader, text=realorange, below=4mm of s2top, anchor=north] (h2)
    {Stage 2 \,--\, Fit the points equation};
  ...
  \node[orangebox, left=of spread] (sim2)
    {\textbf{Sim plays battles}\\[3pt]
     \footnotesize same realistic sim,\\units priced by the equation};
  \node[orangebox, right=of spread] (tweakpts)
    {\textbf{Tune the equation}\\[3pt]
     \footnotesize re-weight stat coefficients\\(and small per-unit residuals)};
  ...
  \node[goalbox, below=10mm of s2bottom, anchor=north] (out)
    {\textbf{Output: one master equation that prices every unit from its stats}\\[3pt]
     \footnotesize (plus small per-unit residuals for the rough edges)\\a fairer 40K --- every unit pulls its weight; games won by skill and balanced lists};
