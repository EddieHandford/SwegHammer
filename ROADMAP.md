...
- **Faction army rules** (10e codex army-rule level):
  ...
  - Death Guard: Disgustingly Resilient (FNP 5+ army-wide) + Contagions of Nurgle / Nurgle's Gift, rebuilt across the durability fidelity audit D fixes: Contagion Range escalates 3"/6"/9" by battle round per the printed schedule (DURA-AUDIT-D1); always-on Afflicted -1 Toughness from round 1 (DURA-AUDIT-D2); one chosen Plague active from round 1 for the whole battle — Skullsquirm Blight (-1 to the Afflicted unit's own Hit roll), Rattlejoint Ague (-1 Save characteristic), or Scabrous Soulrot (-1 Move / -1 Leadership / -1 Objective Control, floored at 1) — picked by an artificial-intelligence heuristic reading the opposing roster at battle start, not a player decision (DURA-AUDIT-D3, `Battle._choose_dg_plague`)
