...
\begin{itemize}[label={},leftmargin=*,itemsep=2pt]
  ...
  \item \done \eddie\ Add \texttt{move: float} (inches) to
    \texttt{UnitProfile}.
  \item \wip \eddie\ Implement \texttt{MovementPhase} -- first cut done
    (each activation is a move-then-shoot sub-phase pair; movement
    targets the \emph{nearest} enemy and clamps to map / aborts into
    impassable terrain). Still missing: objective-aware play, kiting
    when outranged, segment-vs-impassable-terrain collision (we only
...
\begin{itemize}[label={},leftmargin=*,itemsep=2pt]
  ...
  \item \done \eddie\ \texttt{Map} dataclass: board size, terrain list,
    deployment-zone depth. Objective markers still todo.
...
\begin{itemize}[label={},leftmargin=*,itemsep=2pt]
  ...
  \item \done \eddie\ Matplotlib static-frame renderer
    ...
    event stream; feeds the Streamlit replay slider page.
\end{itemize}
