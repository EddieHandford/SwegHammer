...
\begin{itemize}[itemsep=2pt]
  \item Mirror-match calibration (Space Marine vs Space Marine) still
    ...
    \textbf{Needs revising:} this threshold predates objective scoring,
    Battleshock, the charge / melee phases, and the strategy layer.
    The current combat model produces a residual mirror-match bias on
    symmetric terrain (around $\pm 10\%$ over 400 battles) driven by
    objective-contest jitter and movement-heuristic asymmetries. The
    gate stands as ``no worse than the current baseline'' until the
    threshold is renegotiated against the new combat model.
  \item \texttt{python run.py --cli} still runs end-to-end (exit 0).
