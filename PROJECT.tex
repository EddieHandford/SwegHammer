...
faction-specific mechanics that distinguish factions in real play. A
faction-blind simulator cannot reproduce a meta where Necrons hit 53\%
and Aeldari hit 44\%. Headline metric: MAE vs the May 2026 Warp Friends
tournament aggregate, currently $5.11$ pts, target $\le 2.0$ pts. See
\texttt{scripts/evaluate\_vs\_meta.py}.

\subsection*{Goal B --- Equal-quality simulation per faction}
Bring every faction to the same depth of in-game representation. The two
big knobs are stratagems (CP-priced effects) and leader auras (CHARACTER
attached to bodyguard unit), with the per-detachment enhancement system
as a follow-up. Universal Core Stratagems plus three detachment-specific
sets shipped; $\sim18$ detachments still have empty stratagem tuples.
...
\begin{itemize}[itemsep=2pt]
  ...
  \item \textbf{Equilibrium} (\texttt{code/equilibrium.py}, Ed's solver)
    ...
    matrix. Fast, exposes pairwise structure. Phases 1 (shooting) and 2
    (shoot + melee) shipped; Phases 3--6 planned.
\end{itemize}
...
points value. Collapsed into Equilibrium Phase 4 as the
\texttt{tactical\_value(u)} term added post-LSQ.
...
\begin{itemize}[itemsep=2pt]
  ...
  \item \todo\ \textbf{Phase 3} --- defensive integration audit
    (high-FNP low-wound edge cases, MW-vulnerability).
  \item \todo\ \textbf{Phase 4} --- tactical-utility term
    $\text{tactical\_value}(u)$ for non-damaging abilities. This is
    \textbf{Goal D}.
  \item \todo\ \textbf{Phase 5} --- meta-weighting (weight residuals
    by $P(\text{facing } j)$ from tournament data).
  \item \todo\ \textbf{Phase 6} --- solve the actual zero-sum game
    (mixed strategies on the simplex) to handle rock-paper-scissors
    mispricing.
\end{itemize}
...
\textbf{MAE 5.11 pts}, target $\le 2.0$ pts. The MAE bounces as Goal A
fills in --- each faction-specific mechanic (Reanimation, Shadow in the
Warp, WAAAGH! \& Mob Rule) perturbs the matrix until the next mechanic
lands. The aim is monotone-decreasing MAE per faction-mechanic landed,
\emph{not} a smooth descent (the matrix is too coarse for that).
