...
# ---------------------------------------------------------------------------
#
# Moved verbatim to code/sim/geometry.py (Stage A of
# docs/SIM_MODULARIZATION_PLAN.md). Re-imported below (after the constants
# import they depend on) so this module's public surface is unchanged —
# `from code.simulator import _distance, _move_toward, _OccupantGrid,
# _collision_pos_legal, _enemy_path_cap_t` (used by tests and the melee /
# collision diagnostics) keeps resolving. The re-import lives just after the
# `from .sim.constants import ...` line so the constants are bound first.
...
# Module-level constants and instrument statistics moved verbatim to
# code/sim/constants.py (Stage A of docs/SIM_MODULARIZATION_PLAN.md). They are
# re-imported here so this module's public surface is unchanged — both
# `from code.simulator import MAX_ROUNDS` and `simulator.OCFLIP_STATS` keep
# resolving. The instrument dictionaries are re-imported (not copied), so the
# diagnostics that reset `simulator.OCFLIP_STATS` / `BOARDCONTROL_STATS` / etc.
# and the geometry helpers that mutate PATHFIND_STAGE0_STATS / REACH_STATS all
# operate on the same single objects.
from .sim.constants import (  # noqa: F401  (re-exported for the public surface)
    MAX_ROUNDS,
    CP_BONUS_DIVISOR,
    CP_BONUS_CAP,
    COHERENCY_INCHES,
...
from .sim.geometry import (  # noqa: F401  (re-exported for the public surface)
    _distance,
    _OccupantGrid,
...
class Battle:
    ...
    def _decide_winner(
        ...
        # it awards close games to durable survivors. Gated SWEG_VP_TIE_DRAW.
        # Cited as simulator.vp_tie_draw.
        if __import__("os").environ.get("SWEG_VP_TIE_DRAW", "0") == "1":
            return None  # faithful: tied victory points is a draw
        # Legacy surviving-points fall-back (gate off, byte-identical).
        if a_pts > b_pts * 1.10:
            return self.a.name
        if b_pts > a_pts * 1.10:
            return self.b.name
