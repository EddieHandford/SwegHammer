...
SKYSPLINTER_ASSAULT = Detachment(
    ...
    notes=(
        ...
        "gain [LANCE]. EMBARK-V1 (2026-05-28) — the simulator NOW has "
        "an embark / disembark cycle (see simulator._embark / "
        "_disembark / _destroyed_transport_disembark and the matching "
        "embarked-passenger gates in _do_move / _do_shoot / _do_charge "
        "/ _do_fight). The per-weapon LANCE wound-roll bonus and the "
        "per-target ignores-cover modifier on the disembark turn are "
        "STILL missing — wave-45 work. The previous implementation "
        "granted army-wide reroll_wound_ones=True as a proxy; that "
        "proxy was a strict over-buff (always-on, every wound roll, "
        "every unit, with no disembark / transport gate) and drove "
        "the +39.5pt Drukhari overperformance in the May 2026 "
        "calibration. Removed (SC5-1, 2026-05-20). When the LANCE / "
        "IGNORES-COVER per-weapon gating lands, the rule should be "
        "wired narrowly (disembark-turn only, per-unit, weapon-"
        "keyword gated) via the existing _disembarked_this_round set. "
