...
from .sim.constants import MAX_ROUNDS
...
def _charge_field_post_denominator(attacker, target_unit, target_expected_wounds,
    ...
    SWEG_THREAT_ALLOC (default OFF, byte-identical off): this loop builds the
    same per-pair math as `_threat_field_at` but does not route through it,
    so the allocation gate is wired HERE as well (docs/DECISION_LEDGER.md
    "THREAT-FIELD PARTICIPATION RATE" — registered so consumer screen (a),
    SWEG_THREAT_CHARGE on the allocated field, is runnable). When on, each
    enemy's contribution is multiplied by the same attractiveness-
    proportional weight and measured attack propensity the field applies,
    reusing the cached `_threat_alloc_denominators` machinery; the target's
    own tracked contribution is the transformed one, so the kill-probability
    subtraction stays in the same currency as the field it is subtracted
    from. Gate read once per call (cached per battle via _threat_alloc_on),
    never in the loop."""
...
# Behind Enemy Lines forward-zone yield per remaining scoring round (the sim's
# simplified Tactical pool pays a few victory points for a body in the enemy
# deployment zone). Kept modest so the primary marker term dominates the ranking.
_VALUE_BEL_VP_PER_ROUND = 2.0

# Shadow diagnostic counters for the mechanism check (SWEG_VALUE_MOVE_DIAG): in a
# gate-OFF battle, at every objective-destination decision, count how often the
# value ranking WOULD re-route the unit to a different marker than the legacy
# score — with ZERO RNG divergence, because the battle still acts on the legacy
# pick. Mirrors the SWEG_THREAT_CHARGE_DIAG shadow counter exactly.
_VALUE_MOVE_DIAG = {"decisions": 0, "changes": 0}


def reset_value_move_diag() -> None:
    _VALUE_MOVE_DIAG["decisions"] = 0
    _VALUE_MOVE_DIAG["changes"] = 0
...
def _job_channel_kill(unit, friendly, battle, enemy_alive, map_, srr,
    ...
    def _threat_ctx():
        ...
        if threat_rows is None:
            ...
            if alloc_on:
                # Empty denominators when the unit has no army context —
