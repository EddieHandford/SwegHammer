...
cached per board state) — and by the MEASURED opportunity-conditioned
attack-propensity step curve (`strategy._attack_propensity`: the probability
a living activated enemy roster slot attempts output, conditioned on the
enemy's own best expected-wounds opportunity, fitted from the zero-damage
participation decomposition in `scripts/diag_fire_allocation.py` on the
seed-base-0 battery — 0.003 at zero opportunity, then 0.393 / 0.525 / 0.510 /
0.614 by rising opportunity bucket; the measured inversion between the third
and fourth buckets is kept as measured, not smoothed, and the enemy's
opportunity at field-evaluation time is the largest of its cached per-pair
denominator terms). An enemy with one eligible target therefore sends
everything at it — the allocation weight degenerates to one in the isolated
case, though the propensity still applies, because an isolated target is only
shot if the enemy attempts output at all — while an enemy with several splits
the priced risk across them. The realized
